...
} = require("../utility/transporter");

let tempUsers = {}; // In-memory store for temporary users
...
exports.userController = {
  ...
  resetPassword: async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(401)
          .json({ message: "Invalid or expired token", code: 1 });
      }
  ...
  verify : async (req, res) => {
    ...
    try {
        ...
        user.verified = true; // Set verified status

        await user.save(); // Save the user to the database
        delete tempUsers[token]; // Remove from temporary storage
  ...
  signUp: async (req, res) => {
    ...
    try {
      const token = createToken(req.body.email); // Use email as identifier for simplicity
      tempUsers[token] = { ...req.body, verificationToken: token };

      const verificationLink = `${config.servrt_url}/users/verify/${token}`;
      await sendEmailSignUp(req.body.email, req.body.name, verificationLink);
    ...
    try {
      ...
      if (user.active == false) {
        return res
          .status(401)
          .json({
            msg: "your account is blocked. Please contact the site administrator",
            code: 3,
          });
      }
  ...
  chackJoinRequest: async (req, res) => {
    try {
      ...
      for (const eventId of userInfo.my_created_events) {
        let { join_requests, active } = await EventModel.findOne({
          _id: eventId,
        });
    ...
    try {
      ...
      for (const eventId of userInfo.my_created_events) {
        ...
        if (event.join_requests.length > 0) {
          ...
          const joinRequests = await UserModel.find({
            _id: { $in: event.join_requests },
          }).select({
            _id: 1,
            nick_name: 1,
            profile_image: 1,
  ...
  changeActive: async (req, res) => {
    ...
    try {
      ...
      if (userID == req.tokenData._id) {
        return res
          .status(401)
          .json({ msg: "You cant change your Active status" });
      }
      let data = await UserModel.updateOne(
        { _id: userID },
  ...
  deleteUser: async (req, res) => {
    try {
      ...
      if (req.tokenData._id == delId) {
        return res
          .status(401)
          .json({ msg: "You can't delete your own acount" });
      }
